  ...
  for await (moment of timeline) {
    if (isMomentRecent(moment.created_at)) {
      if (moment.event == 'cross-referenced' && isLinkedIssue(moment, issueNum)) {
        return false
      } else if (moment.event == 'commented' && isCommentByAssignee(moment, assignees)) {
        return false
      }
    }
  }
...
function isCommentByAssignee(data, assignees) {
...
function formatComment(assignees) {
  const path = './update-instructions-template.md'
